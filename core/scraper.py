...
scrape_status: dict = {
    ...
    "stop_requested": False, "progress_id": None
}
...
def _log(msg: str):
    ...
    if len(scrape_status["log_messages"]) > _MAX_LOGS:
        scrape_status["log_messages"] = scrape_status["log_messages"][-_MAX_LOGS:]
    print(msg)
    ...
    try:
        ...
        try:
            ...
            if prev_progress:
                ...
                scrape_status["progress_id"] = prev_progress["id"]
                ...
                cursor_dict.execute("UPDATE crawl_progress SET status='running' WHERE id=%s", (prev_progress["id"],))
                conn.commit()
            else:
                cursor_dict.execute(
                ...
                conn.commit()
                scrape_status["progress_id"] = cursor_dict.lastrowid
        ...
        for yr in range(request.start_year, request.end_year + 1):
            if scrape_status.get("stop_requested"): break
            target_year = str(yr)
            ...
            for issue_info in issue_links:
                ...
                try:
                    ...
                    # Cố gắng lấy tên tập từ h1 nếu base_issue_name quá ngắn (ví dụ chỉ có năm)
                    ...
                    # We will create the final issue_dir later after detecting language
                    # issue_dir = os.path.join(output_folder, target_year, issue_name)
        ...
        try:
            ...
            # Cập nhật bảng crawl_progress
            if scrape_status.get("progress_id"):
                final_status = 'paused' if scrape_status.get("stop_requested") else 'completed'
                log_cursor.execute(
                    "UPDATE crawl_progress SET success_count=%s, duplicate_count=%s, skipped_count=%s, status=%s WHERE id=%s",
                    (scrape_status["success"], scrape_status["duplicates"], scrape_status["skipped"], final_status, scrape_status["progress_id"])
                )
                
            # Lưu lịch sử khi hoàn thành
            if not scrape_status.get("stop_requested"):
                log_cursor.execute(
